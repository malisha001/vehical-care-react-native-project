...
import { BillItem, CleaningBooking } from "../types";
...
const serviceName = (booking: CleaningBooking) =>
  typeof booking.serviceId === "object" ? booking.serviceId.name : "Cleaning Service";

const serviceDuration = (booking: CleaningBooking) =>
  typeof booking.serviceId === "object" ? booking.serviceId.duration : undefined;

const servicePrice = (booking: CleaningBooking) =>
  booking.baseServicePrice ??
  (typeof booking.serviceId === "object" ? booking.serviceId.price : 0) ??
  0;

const userName = (booking: CleaningBooking) =>
  typeof booking.userId === "object" ? booking.userId.name : "-";

const userEmail = (booking: CleaningBooking) =>
...
const billRows = (booking: CleaningBooking): BillItem[] => [
  {
    description: serviceName(booking),
    price: servicePrice(booking),
  },
  ...(booking.billItems || []),
];
...
const CleaningBillPage: React.FC = () => {
  const { id } = useParams();
  const navigate = useNavigate();

  const { data: booking, isLoading } = useQuery({
    queryKey: ["cleaning-booking-bill", id],
    enabled: !!id,
    queryFn: () => cleaningBookingApi.getById(id!).then((r) => r.data.data),
  });
  ...
  const printBill = () => {
    ...
    doc.setFontSize(14);
    doc.text("Cleaning Service Bill", 14, 25);
    doc.setFontSize(10);
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        ["Vehicle", vehicleText(booking), "Booking Status", booking.status],
        ["Slot Date", booking.date, "Time Slot", booking.timeSlot],
        ["Service", serviceName(booking), "Duration", serviceDuration(booking) || "-"],
        ["Customer Notes", booking.notes || "-", "Admin Notes", booking.adminNotes || "-"],
      ],
    ...
    doc.save(`cleaning-bill-${booking._id}.pdf`);
  };
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <button
            onClick={() => navigate("/cleaning-bookings")}
            className="text-sm font-medium text-primary-700 hover:text-primary-800"
          >
            Back to cleaning bookings
          </button>
      ...
      <div className="card">
        ...
        <div className="grid gap-4 py-5 md:grid-cols-4">
          ...
          />
          <Detail label="Service" value={serviceName(booking)} />
          <Detail label="Duration" value={serviceDuration(booking)} />
          <Detail label="Slot Date" value={booking.date} />
          <Detail label="Time Slot" value={booking.timeSlot} />
        </div>

        <div className="grid gap-4 border-t border-gray-100 pt-5 md:grid-cols-2">
          <Detail label="Customer Notes" value={booking.notes} />
...
export default CleaningBillPage;
